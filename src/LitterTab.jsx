// LitterTab.tsx — IziPaws
// Cách dùng trong DogProfile.tsx:
...
const VACCINE_SCHEDULE = {"C5":{intervalDays:365,label:"Annual"},"C3":{intervalDays:365,label:"Annual"},"Rabies":{intervalDays:365,label:"Annual"},"Puppy 1st":{intervalDays:28,label:"4 weeks"},"Puppy 2nd":{intervalDays:28,label:"4 weeks"},"Puppy Final":{intervalDays:365,label:"Annual after final"},"Kennel Cough":{intervalDays:365,label:"Annual"}};
const WORMING_SCHEDULE = {"Milbemax":{intervalDays:90,label:"Every 3 months"},"Drontal":{intervalDays:90,label:"Every 3 months"},"Interceptor":{intervalDays:30,label:"Monthly"},"Heartgard":{intervalDays:30,label:"Monthly"},"Panoramis":{intervalDays:30,label:"Monthly"},"Nexgard Spectra":{intervalDays:30,label:"Monthly"},"Other":{intervalDays:90,label:"Every 3 months"}};

const genId = () => Date.now().toString(36).toUpperCase() + Math.random().toString(36).slice(2,5).toUpperCase();
const addDays = (d,n) => { if(!d)return""; const x=new Date(d); x.setDate(x.getDate()+n); return x.toISOString().split("T")[0]; };
const formatDate = (d) => { if(!d)return""; const [y,m,day]=d.split("-"); return `${day}-${m}-${y}`; };
const daysUntil = (d) => { if(!d)return null; return Math.ceil((new Date(d).getTime()-new Date().setHours(0,0,0,0))/(1000*60*60*24)); };
const statusStyle = (s) => ({Available:{bg:"#E1F5EE",color:"#0F6E56"},Reserved:{bg:"#FAEEDA",color:"#633806"},Sold:{bg:"#FCEBEB",color:"#A32D2D"},Kept:{bg:"#EEEDFE",color:"#3C3489"}}[s]);
const emptyBuyer = () => ({name:"",phone:"",email:"",address:"",notes:""});
const emptyPuppy = () => ({name:"",gender:"",colour:"",collarColour:"",weight:"",photo:"",status:"Available",buyer:emptyBuyer(),vaccines:[],wormRecords:[],documents:[]});
const emptyLitter = () => ({litterId:"",dob:"",sire:"",dam:"",maleCount:"",femaleCount:"",notes:""});
const IS = {width:"100%",boxSizing:"border-box",padding:"8px 10px",borderRadius:"var(--border-radius-md)",border:"1px solid var(--color-border-secondary)",background:"var(--color-background-primary)",color:"var(--color-text-primary)",fontSize:13,outline:"none"};
const lbl = (t) => <div style={{fontSize:12,color:"var(--color-text-secondary)",marginBottom:4}}>{t}</div>;

function PuppyCard({puppy,litterId,litters,onChange}) {
  ...
  const updatePuppy = (patch) => {
    onChange(litters.map(l=>l.id===litterId?{...l,puppies:l.puppies.map(p=>p.id===puppy.id?{...p,...patch}:p)}:l));
  };
  const deletePuppy = () => {
    onChange(litters.map(l=>l.id===litterId?{...l,puppies:l.puppies.filter(p=>p.id!==puppy.id)}:l));
  };
  const handlePhoto = (e) => {
    const f=e.target.files?.[0]; if(!f)return;
    const r=new FileReader(); r.onload=ev=>updatePuppy({photo:ev.target.result}); r.readAsDataURL(f);
  };
  const handleDoc = (e) => {
    const f=e.target.files?.[0]; if(!f)return;
  ...
  const addVaccine = () => {
    if(!newVax.name||!newVax.date)return;
    updatePuppy({vaccines:[...puppy.vaccines,{id:genId(),...newVax}]});
    setNewVax({name:"",date:"",nextDate:""}); setShowAddVax(false);
  };
  const addWorm = () => {
    if(!newWorm.name||!newWorm.date)return;
    updatePuppy({wormRecords:[...puppy.wormRecords,{id:genId(),...newWorm}]});
    setNewWorm({name:"",date:"",nextDate:"",notes:""}); setShowAddWorm(false);
  };

  const st = statusStyle(puppy.status);
  const SUBTABS = [
    {k:"info",label:"Info"},
  ...
  return (
    <div style={{border:"1px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-lg)",overflow:"hidden",background:"var(--color-background-primary)"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",cursor:"pointer",userSelect:"none"}} onClick={()=>setOpen(o=>!o)}>
        ...
        <span style={{fontSize:11,padding:"3px 8px",borderRadius:99,fontWeight:500,background:st.bg,color:st.color,flexShrink:0}}>{puppy.status}</span>
        <span style={{fontSize:14,color:"var(--color-text-tertiary)",transform:open?"rotate(180deg)":"none",transition:"transform 0.18s"}}>▾</span>
        <button onClick={e=>{e.stopPropagation();if(confirm(`Delete ${puppy.name||"this puppy"}?`))deletePuppy();}} style={{background:"none",border:"none",cursor:"pointer",color:"var(--color-text-tertiary)",fontSize:16,padding:"0 2px"}}>✕</button>
      </div>
      ...
      {open&&(
        <div style={{borderTop:"1px solid var(--color-border-tertiary)",padding:"12px 14px",background:"var(--color-background-secondary)"}}>
          <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap"}}>
            {SUBTABS.map(t=>(
              <button key={t.k} onClick={()=>setSubTab(t.k)} style={{padding:"5px 10px",borderRadius:"var(--border-radius-md)",fontSize:11,cursor:"pointer",border:subTab===t.k?"1.5px solid #534AB7":"1.5px solid var(--color-border-tertiary)",background:subTab===t.k?"#EEEDFE":"var(--color-background-primary)",color:subTab===t.k?"#3C3489":"var(--color-text-secondary)"}}>{t.label}</button>
            ))}
          </div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              <div style={{display:"flex",alignItems:"center",gap:12}}>
                ...
                <div>
                  ...
                  {puppy.photo&&<button onClick={()=>updatePuppy({photo:""})} style={{marginLeft:8,padding:"6px 10px",borderRadius:"var(--border-radius-md)",fontSize:12,border:"1px solid #F09595",background:"none",color:"#E24B4A",cursor:"pointer"}}>Remove</button>}
                </div>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                ...
                <div>{lbl("Status")}<select value={puppy.status} onChange={e=>updatePuppy({status:e.target.value})} style={IS}>{PUPPY_STATUSES.map(s=><option key={s}>{s}</option>)}</select></div>
              </div>
              ...
              {(puppy.status==="Reserved"||puppy.status==="Sold")&&(
                <div style={{background:"var(--color-background-primary)",borderRadius:"var(--border-radius-md)",border:"1px solid var(--color-border-tertiary)",overflow:"hidden"}}>
                  <div style={{padding:"10px 12px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}} onClick={()=>setShowBuyer(o=>!o)}>
                    <div style={{fontSize:12,fontWeight:500,color:"var(--color-text-secondary)"}}>👤 Buyer Information {puppy.buyer.name&&`— ${puppy.buyer.name}`}</div>
                  ...
                  {showBuyer&&(
                    <div style={{padding:"0 12px 12px",display:"flex",flexDirection:"column",gap:8,borderTop:"1px solid var(--color-border-tertiary)"}}>
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                        ...
                        <div>{lbl("Email")}<input value={puppy.buyer.email} onChange={e=>updatePuppy({buyer:{...puppy.buyer,email:e.target.value}})} placeholder="jane@email.com" type="email" style={IS}/></div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
                ...
                return(
                  <div key={v.id} style={{background:"var(--color-background-primary)",border:`1px solid ${ov?"#F09595":sn?"#FAC775":"var(--color-border-tertiary)"}`,borderRadius:"var(--border-radius-md)",padding:"9px 12px",display:"flex",justifyContent:"space-between",alignItems:"center",opacity:v.completed?0.6:1}}>
                    ...
                    <div style={{display:"flex",gap:6}}>
                      {!v.completed&&<button onClick={()=>updatePuppy({vaccines:puppy.vaccines.map((x,j)=>j===i?{...x,completed:true}:x)})} style={{background:"none",border:"1px solid #1D9E75",borderRadius:6,cursor:"pointer",color:"#1D9E75",fontSize:11,padding:"3px 8px"}}>✓ Done</button>}
                      <button onClick={()=>updatePuppy({vaccines:puppy.vaccines.filter((_,j)=>j!==i)})} style={{background:"none",border:"none",cursor:"pointer",color:"var(--color-text-tertiary)",fontSize:16}}>✕</button>
                    </div>
                  </div>
                );
              })}
              {showAddVax?(
                <div style={{background:"var(--color-background-primary)",borderRadius:"var(--border-radius-md)",padding:"12px",display:"flex",flexDirection:"column",gap:8,border:"1px solid var(--color-border-tertiary)"}}>
                  {lbl("Vaccine Name")}
                  <input value={newVax.name} onChange={e=>{const name=e.target.value;const sc=VACCINE_SCHEDULE[name];setNewVax(p=>({...p,name,nextDate:sc&&p.date?addDays(p.date,sc.intervalDays):p.nextDate}));}} placeholder="C5, Puppy 1st..." list="pup-vax-list" style={IS}/>
                  <datalist id="pup-vax-list">{Object.keys(VACCINE_SCHEDULE).map(v=><option key={v} value={v}/>)}</datalist>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                    ...
                    <div>{lbl("Next Due")}<input type="date" value={newVax.nextDate} onChange={e=>setNewVax(p=>({...p,nextDate:e.target.value}))} style={{...IS,border:VACCINE_SCHEDULE[newVax.name]&&newVax.date?"1.5px solid #AFA9EC":IS.border,background:VACCINE_SCHEDULE[newVax.name]&&newVax.date?"#EEEDFE":IS.background}}/>{VACCINE_SCHEDULE[newVax.name]&&newVax.date&&<div style={{fontSize:10,color:"#534AB7",marginTop:2}}>Auto: {VACCINE_SCHEDULE[newVax.name].label}</div>}</div>
                  </div>
              ...
              {puppy.wormRecords.map((w,i)=>{
                ...
                return(
                  <div key={w.id} style={{background:"var(--color-background-primary)",border:`1px solid ${ov?"#F09595":sn?"#FAC775":"var(--color-border-tertiary)"}`,borderRadius:"var(--border-radius-md)",padding:"9px 12px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    ...
                    <button onClick={()=>updatePuppy({wormRecords:puppy.wormRecords.filter((_,j)=>j!==i)})} style={{background:"none",border:"none",cursor:"pointer",color:"var(--color-text-tertiary)",fontSize:16}}>✕</button>
                  </div>
                );
              })}
              {showAddWorm?(
                <div style={{background:"var(--color-background-primary)",borderRadius:"var(--border-radius-md)",padding:"12px",display:"flex",flexDirection:"column",gap:8,border:"1px solid var(--color-border-tertiary)"}}>
                  {lbl("Product")}
                  <input value={newWorm.name} onChange={e=>{const name=e.target.value;const sc=WORMING_SCHEDULE[name];setNewWorm(p=>({...p,name,nextDate:sc&&p.date?addDays(p.date,sc.intervalDays):p.nextDate}));}} placeholder="Milbemax, Drontal..." list="pup-worm-list" style={IS}/>
                  <datalist id="pup-worm-list">{Object.keys(WORMING_SCHEDULE).map(v=><option key={v} value={v}/>)}</datalist>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                    ...
                    <div>{lbl("Next Due")}<input type="date" value={newWorm.nextDate} onChange={e=>setNewWorm(p=>({...p,nextDate:e.target.value}))} style={{...IS,border:WORMING_SCHEDULE[newWorm.name]&&newWorm.date?"1.5px solid #AFA9EC":IS.border,background:WORMING_SCHEDULE[newWorm.name]&&newWorm.date?"#EEEDFE":IS.background}}/>{WORMING_SCHEDULE[newWorm.name]&&newWorm.date&&<div style={{fontSize:10,color:"#534AB7",marginTop:2}}>Auto: {WORMING_SCHEDULE[newWorm.name].label}</div>}</div>
                  </div>
                  <div>{lbl("Notes")}<input value={newWorm.notes} onChange={e=>setNewWorm(p=>({...p,notes:e.target.value}))} placeholder="Weight, dose, reactions..." style={IS}/></div>
                  <div style={{display:"flex",gap:8}}>
          ...
          {subTab==="docs"&&(
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              ...
              {puppy.documents.map(doc=>(
                <div key={doc.id} style={{background:"var(--color-background-primary)",border:"1px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-md)",padding:"9px 12px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    <div style={{fontSize:13}}>{doc.fileType.includes("pdf")?"📄":doc.fileType.includes("image")?"🖼️":"📎"} {doc.name}</div>
                    <div style={{fontSize:11,color:"var(--color-text-secondary)",marginTop:2}}>{doc.docType} · {doc.date}</div>
                  </div>
                  <div style={{display:"flex",gap:6}}>
                    <a href={doc.url} download={doc.name} style={{fontSize:11,padding:"3px 8px",borderRadius:6,border:"1px solid var(--color-border-secondary)",color:"var(--color-text-secondary)",textDecoration:"none"}}>⬇</a>
                    <button onClick={()=>updatePuppy({documents:puppy.documents.filter(d=>d.id!==doc.id)})} style={{background:"none",border:"none",cursor:"pointer",color:"var(--color-text-tertiary)",fontSize:16}}>✕</button>
                  </div>
                </div>
              ))}
...
export default function LitterTab({litters,damName,onChange}) {
  const [showAddLitter,setShowAddLitter] = useState(false);
  const [newLitter,setNewLitter] = useState(emptyLitter());
  const [expandedIds,setExpandedIds] = useState(new Set());
  const [addPuppyFor,setAddPuppyFor] = useState(null);
  const [newPuppy,setNewPuppy] = useState(emptyPuppy());

  const toggleLitter = (id) => setExpandedIds(prev=>{const n=new Set(prev);n.has(id)?n.delete(id):n.add(id);return n;});

  const addLitter = () => {
    if(!newLitter.dob)return;
    const litter={id:genId(),litterId:newLitter.litterId||`LIT-${genId()}`,dob:newLitter.dob,sire:newLitter.sire,dam:newLitter.dam||damName,maleCount:newLitter.maleCount,femaleCount:newLitter.femaleCount,notes:newLitter.notes,puppies:[]};
    onChange([...litters,litter]);
    setNewLitter(emptyLitter()); setShowAddLitter(false);
    setExpandedIds(prev=>new Set([...prev,litter.id]));
  };

  const deleteLitter = (id) => {
    if(!confirm("Delete this litter and all its puppies?"))return;
    onChange(litters.filter(l=>l.id!==id));
  };

  const addPuppy = (litterId) => {
    if(!newPuppy.name)return;
    onChange(litters.map(l=>l.id===litterId?{...l,puppies:[...l.puppies,{id:genId(),...newPuppy}]}:l));
    setNewPuppy(emptyPuppy()); setAddPuppyFor(null);
  };

  const stats = (l) => ({
    avail:l.puppies.filter(p=>p.status==="Available").length,
    reserved:l.puppies.filter(p=>p.status==="Reserved").length,
    sold:l.puppies.filter(p=>p.status==="Sold").length,
    kept:l.puppies.filter(p=>p.status==="Kept").length,
  });

  return (
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      {litters.length===0&&!showAddLitter&&(
        <div style={{textAlign:"center",padding:"36px 0",color:"var(--color-text-tertiary)",fontSize:13}}>
          <div style={{fontSize:32,marginBottom:8}}>🐾</div>No litters recorded yet
        </div>
      )}
          ...
          <div key={litter.id} style={{border:"1px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-lg)",overflow:"hidden"}}>
            ...
            {exp&&(
              <div style={{padding:"14px",background:"var(--color-background-primary)",display:"flex",flexDirection:"column",gap:10}}>
                ...
                {litter.puppies.map(puppy=><PuppyCard key={puppy.id} puppy={puppy} litterId={litter.id} litters={litters} onChange={onChange}/>)}
                ...
                {addPuppyFor===litter.id?(
                  <div style={{background:"var(--color-background-secondary)",borderRadius:"var(--border-radius-md)",padding:"12px",border:"1px solid var(--color-border-tertiary)",display:"flex",flexDirection:"column",gap:10}}>
                    ...
                    <div style={{display:"flex",gap:8}}>
                      ...
                      <button onClick={()=>{setAddPuppyFor(null);setNewPuppy(emptyPuppy());}} style={{flex:1,padding:"8px",borderRadius:"var(--border-radius-md)",border:"1px solid var(--color-border-secondary)",background:"var(--color-background-primary)",color:"var(--color-text-secondary)",fontSize:13,cursor:"pointer"}}>Cancel</button>
                    </div>
                  </div>
                ):(
                  <button onClick={()=>{setAddPuppyFor(litter.id);setNewPuppy(emptyPuppy());}} style={{width:"100%",padding:"9px",borderRadius:"var(--border-radius-md)",border:"1.5px dashed var(--color-border-secondary)",background:"var(--color-background-primary)",color:"var(--color-text-secondary)",fontSize:13,cursor:"pointer"}}>+ Add Puppy</button>
                )}
              </div>
            )}
          </div>
      ...
      {/* Add Litter */}
      {showAddLitter?(
        <div style={{background:"var(--color-background-secondary)",borderRadius:"var(--border-radius-lg)",padding:"14px",border:"1px solid var(--color-border-tertiary)",display:"flex",flexDirection:"column",gap:10}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            ...
            <div>{lbl("Sire (Father)")}<input value={newLitter.sire} onChange={e=>setNewLitter(p=>({...p,sire:e.target.value}))} placeholder="Sire name..." style={IS}/></div>
            <div>{lbl("Dam (Mother)")}<input value={newLitter.dam||damName} onChange={e=>setNewLitter(p=>({...p,dam:e.target.value}))} placeholder={damName||"Dam name..."} style={IS}/></div>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <button onClick={()=>{setShowAddLitter(false);setNewLitter(emptyLitter());}} style={{flex:1,padding:"9px",borderRadius:"var(--border-radius-md)",border:"1px solid var(--color-border-secondary)",background:"var(--color-background-primary)",color:"var(--color-text-secondary)",fontSize:13,cursor:"pointer"}}>Cancel</button>
          </div>
        </div>
      ):(
        <button onClick={()=>setShowAddLitter(true)} style={{width:"100%",padding:"11px",borderRadius:"var(--border-radius-md)",border:"1.5px dashed var(--color-border-secondary)",background:"var(--color-background-primary)",color:"var(--color-text-secondary)",fontSize:13,cursor:"pointer"}}>+ Add Litter</button>
      )}
    </div>
  );
}