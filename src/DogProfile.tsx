...
export default function DogProfile() {
  ...
  const handleExtracted = (data: any) => {
    ...
    if (data.name) {
      const duplicate = dogs.find(d =>
        d.id !== activeDogId &&
        d.name.toLowerCase().trim() === data.name.toLowerCase().trim()
      );
      if (duplicate) {
        if (!window.confirm(`⚠️ "${data.name}" already exists in your profiles.\nDo you still want to apply this data to the current profile?`)) return;
      }
    }
    setDogs(prev => {
